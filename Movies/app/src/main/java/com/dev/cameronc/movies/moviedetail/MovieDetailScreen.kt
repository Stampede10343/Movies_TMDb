                ...
                .subscribe({ movieDetailsState ->
                    when (movieDetailsState) {
                        is ScreenState.Ready<MovieDetails> -> {
                            ...
                            groupieAdapter.add(MovieDetailBackdrop(data.backdropPath, imageDownloader))
                            val runtime = context.getString(R.string.movie_duration, data.runtime.toString())
                            groupieAdapter.add(MovieDetailInfo(data.title, data.rating, runtime, data.voteAverage, data.genres, data.overview))

                            movie_detail_recyclerview.adapter = groupieAdapter
                            movie_detail_recyclerview.layoutManager = LinearLayoutManager(context)

                            groupieAdapter.add(MovieDetailCast(data.cast, imageDownloader))
                            groupieAdapter.add(MovieReviews(data.reviews))
                            groupieAdapter.add(RelatedMoviesItem(data.similarMovies, imageDownloader))
                        }
...
//    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        Timber.i("Measure time: ${System.currentTimeMillis() - startTime}")
    }
